...
function handleClick(event) {
	console.log(event);
	functionName = event.composedPath()[1];
	styleValue = event.target.value;
	botoes = event;
	styleListObject[functionName.id](styleValue);
	showCss();
	removeActive(functionName.id);
	event.composedPath()[0].classList.add(activeClass);
}

// inicia o css com algumas opções já estabelecidas
function initcss() {
	divResult.style.cssText =
		"justify-content: initial; align-items: initial; flex-direction: initial; align-content: initial; flex-wrap : wrap;";
}
initcss();
